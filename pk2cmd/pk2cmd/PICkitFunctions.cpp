...
void CPICkitFunctions::WriteConfigOutsideProgMem()
{
    ...
    for (int i = 0, j = 0; i < configWords; i++)
    {
            ...
            _tcsncmp(DevFile.Families[ActiveFamily].FamilyName, "PIC18/PIC18F MSB1st", 19) == 0) 
        {
            ...
            if (DevFile.PartsList[ActivePart].IgnoreBytes == 0x000C)    // Q40,Q41,Q43,Q83,Q84 configuration is read/written byte at a time
...
//                    }
//                    else {
//                        DeviceBuffers->ProgramMemory[endOfBuffer - cfg] = 0xffff;
//                    }
                }
...
bool CPICkitFunctions::ReadConfigOutsideProgMem(char function)
{
    ...
    for (int word = 0; word < configWords; word++)
    {
        ...
        {
            config = reverse16Bits(config);
            if (DevFile.PartsList[ActivePart].IgnoreBytes == 0x000C)    // Q40,Q41,Q43 configuration is read/written byte at a time
		...
		{ // BLANK_CHECK or VERIFY_MEM
            ...
            if (word < 9) {
                ...
                configVerifyMask = 0x00ff;  // if device has odd number of config bytes, the missing one is indicated by IgnoreAddress.
            }
            else {
...
bool CPICkitFunctions::SearchDevice(int familyIndex)
        {
            ...
            if(_tcsncmp(DevFile.Families[ActiveFamily].FamilyName, "Midrange/1.8V Min MSB1st", 24) == 0)
            //if (DevFile.Families[familyIndex].FamilyName == "Midrange/1.8V Min MSB1st")
            {
                deviceID = reverse16Bits(deviceID);
                deviceID >>= 2;
                //LastDeviceRev = (int)reverse16Bits((uint)(LastDeviceRev));
                //LastDeviceRev >>= 1; // Why this has to be >>1 to get correct rev??? is there something else wrong?
            }
            //if (DevFile.Families[familyIndex].FamilyName == "PIC18/PIC18F MSB1st")
            if (_tcsncmp(DevFile.Families[ActiveFamily].FamilyName, "PIC18/PIC18F MSB1st", 19) == 0)
            {
                deviceID = reverse16Bits(deviceID);
                //LastDeviceRev = (int)reverse16Bits((uint)(LastDeviceRev));
    ...
    if (DevFile.PartsList[ActivePart].ConfigAddr == DevFile.PartsList[ActivePart].UserIDAddr)
    {
        ...
        */
        ...
        if (cfgmem)
        {
            ...
            cfgBuf[0] |= (~(unsigned int)DevFile.PartsList[ActivePart].ConfigMasks[1] & DevFile.PartsList[ActivePart].ConfigBlank[1])
