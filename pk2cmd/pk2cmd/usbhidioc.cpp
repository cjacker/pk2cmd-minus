...
bool CUsbhidioc::FindTheHID(int unitNumber)
{
	...
	const unsigned int pickit2_ProductID = 0x0033;   // PICkit 2 Flash Starter Kit
	const unsigned int pickit3_ProductID = 0x900a;	// PICkit 3
			...
			if ((int)DeviceHandle != -1)
			{
				...
				*/
				...
				if (Result != 0)
				{
					...
					{
						if (Attributes.ProductID == pickit2_ProductID || Attributes.ProductID == pickit3_ProductID)
						{
							...
							if (unitNumber == unitIndex)
							{
								...
								DeviceDetected = true;
								if (Attributes.ProductID == pickit2_ProductID)
									m_type = Pickit2;
								else
									m_type = Pickit3;
